...
from db_connection_setup import mysql_engine, mongodb_conn, neo4j_conn
...
def update_widget_one(value):
    ...
    if not value:
        ...
        trending_keywords = mongo_utils.get_trending_keywords(value, mongodb_conn)
...
def update_widget_two(n_clicks, value):
    ...
    if n_clicks > 0:
        if not value:
            ...
            keyword_performance_over_time = sql_utils.get_keyword_performance_over_time(value, mysql_engine)
...
def update_widget_three(n_clicks, value):
    ...
    if n_clicks > 0:
        if value:
            top_universities_by_keyword = neo_utils.get_top_universities_by_keyword(value, neo4j_conn)
...
def update_widget_four(n_clicks, value_1, value_2):
    ...
    if n_clicks > 0:
        if not value_1 or not value_2:
            ...
            faculty_list_by_university = sql_utils.get_faculty_list_by_university(value_1, value_2, mysql_engine)
...
def update_widget_five(n_clicks, value):
    ...
    if n_clicks > 0:
        if not value:
            ...
            professor_info = mongo_utils.get_professor_info(value, mongodb_conn)
    ...
    if ctx.triggered_id == "widget-6-submit-button":
        if n_clicks > 0:
            if not value:
                ...
                sql_utils.execute_insert_university_pick(value, mysql_engine)
        ...
        if previous is None or current is None:
            ...
            if len(previous) > len(current):
                ...
                if not deleted_row:
                    ...
                    sql_utils.execute_delete_university_pick(deleted_row[0]["id"], deleted_row[0]["ranking"], mysql_engine)
            # Handles update
                ...
                if new_row[0]["ranking"] == old_row[0]["ranking"]:
                    raise PreventUpdate
                sql_utils.execute_update_ranking(new_row[0]["id"], new_row[0]["ranking"], old_row[0]["ranking"], mysql_engine)
            else:
                raise PreventUpdate

    # THREE: Check if we have any data and set big message accordingly, then return
    # Always try to show user's followed universities
    university_picks = sql_utils.get_university_picks(mysql_engine)
...
def update_widget_seven(selected_row, uni_data, n_clicks, value_1, value_2, previous, current, stored_data):
    ...
    if ctx.triggered_id == "widget-6-data":
        if not selected_row or not uni_data:
            ...
            professors_for_university_pick = sql_utils.get_professors_for_university_pick(data["uni_id"], mysql_engine)
        ...
        if ctx.triggered_id == "widget-7-submit-button":
            if n_clicks > 0:
                if not value_1 or not value_2:
                    ...
                    sql_utils.execute_insert_professor_for_university_pick(data["uni_id"], value_1, value_2, mysql_engine)
        # Handles delete & update
            ...
            if previous is None or current is None:
                ...
                if len(previous) > len(current):
                    ...
                    if not deleted_row:
                        ...
                        sql_utils.execute_delete_professor_for_university_pick(data["uni_id"], deleted_row[0]["professor_id"], mysql_engine)
                # Handles update
                elif len(current) == len(previous):
                    new_row = [row for row in current if row not in previous]
                    old_row = [row for row in previous if row not in current]
                    ...
                    if new_row[0]["professor_id"] != old_row[0]["professor_id"]:
                        raise PreventUpdate
                    sql_utils.execute_update_keyword_for_professor(new_row[0]["university_id"], new_row[0]["professor_id"], new_row[0]["research_interest"], mysql_engine)
                else:
                    raise PreventUpdate
        professors_for_university_pick = sql_utils.get_professors_for_university_pick(data["uni_id"], mysql_engine)
        label = f'Professors You\'re Interested in at {data["uni_name"]}'
